...
const EncyclopediaSearchComponent = (props) => {
    ...
    const encyclopediaSearchResults = useSelector((state: RootState) => state.encyclopedia.encyclopedia);
    ...
    const renderSearchList = () => {
        return (
            <FlatList style={{height: Math.floor(deviceHeight*0.7)}} data={encyclopediaSearchResults}
                      renderItem={({item}) => (
                          <TouchableHighlight key={item.url}
                                              activeOpacity={0.6}
